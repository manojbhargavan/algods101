...
```csharp
List<T> result = new List<T>(); // One Step = O(1), in general we do not count this but we will act as if we do.

for (int i = 0; i < n; i++) // n Steps = O(n)
{
    result.Add(value);
}

return result; // One Step = O(1)
```
