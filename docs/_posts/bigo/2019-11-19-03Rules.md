...
![Why Ignore Constants]({{site.baseurl}}/assets/images/bigo/bigocomplexityconstants.jpg)
...
Say we have two different input parameters one for each of the for loop. Now we have two potentially different input sizes say n for input1 and m for input2. Hence the complexity of this cannot be simplified to O(n). It should be O(n + m) since we have two different input sizes.
...
Let us say the number of input in input1 is n and input2 is m. The inner loop one operating on input2 will be executed for each element in input1 m times i.e., this algorithm is of the order O(n*m).

Say both loops are operating on the same input, the size of the input will be obviously same say n. Hence for the below example the complexity is O(n*n) = O(n^2).
...
This rule simply says that we only need to consider the most dominant term while determining the complexity of an algorithm. Say we have a algorithm with the complexity of O(n + n^2), as n grows significantly n^2 outgrows n and n has close to no impact on the operations required to perform the task. Lets chart this and look at how it pans out. 

![Drop non Dominants]({{site.baseurl}}/assets/images/bigo/domination.jpg)

As you see as the input size increases n has no impact on the overall performance/number of operations, hence we can drop n of the equation. A final example which shows how constants and non dominant terms can be dropped.

Consider O(n^2 + 3x + 1000 + n/2) vs O(n^2), clearly need I say more.
![Drop non Dominants]({{site.baseurl}}/assets/images/bigo/domination2.jpg)

[Next >> Constant Time O(1) Example]({{site.baseurl}}{% post_url ./bigo/2019-11-20-04ConstantTime %})
